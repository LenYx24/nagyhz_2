...
namespace Menu {
MenuButton::MenuButton(Resources::Holder &h, sf::String str,
...
void onclick_start(StateManager &s) { s.PushState(new ModeSelectionState); }

void onclick_back(StateManager &s) { s.PopState(); }
MainState::MainState() {
    ...
    for (sf::Event event = sf::Event{}; renderer.PollEvent(event);) {
        if (event.type == sf::Event::Closed) {
            ...
            for (Button b : buttons) {
                if (b.getGlobalBounds().contains(event.mouseButton.x,
                                                 event.mouseButton.y)) {
                    b.onclick(s);
                }
            }
